...
def interact_with_wormgpt(user_input, friendship_history):
    try:
        ...
        print("Mo3az: Soz, I couldn't process your request. Please try again.")
        return ""
...
def main():
    print(f"{LIGHT_BLUE}Mo3az:{ENDC} Hello Friend")
    ...
    while True:
        try:
            ...
            if is_gibberish(user_input):
                print(f"{LIGHT_BLUE}Mo3az:{ENDC} Please provide more info ;(")
                continue
            ...
            if user_input.lower() == last_user_input:
                print(f"{LIGHT_BLUE}Mo3az:{ENDC} Stop being annoying.")
                continue
            ...
            if any(word in user_input.lower() for word in gratitude_words):
                print(f"{LIGHT_BLUE}Mo3az:{ENDC}", random.choice(gratitude_responses))
                continue

            response = interact_with_wormgpt(user_input, friendship_history)

            if not response:
                print(f"{LIGHT_BLUE}Mo3az:{ENDC} Yeah, What? (you broke it lolz, enter text to reset hopefully)")
            elif response.lower() != user_input.lower():
                print(f"{LIGHT_BLUE}Mo3az:{ENDC}", response)
            ...
            print(f"\n{LIGHT_BLUE}Mo3az:{ENDC} Goodbye! Exiting...")
            save_friendship_history(friendship_history)
            break
